...
class User extends Frontend
{
    ...
    public function comment() {
        ...
        $message = $messageModel->getMessageByUid($user['id']);
        foreach($message as $key=>$vo){
            if ($message[$key]['type'] == 'news') {
                $temp = $newsModel->getNewById($vo['scenic_id']);
            } else {
                $temp = $strategyModel->getStrategyById($vo['scenic_id']);
            }
            $message[$key]['title'] = $temp['title'];
            $message[$key]['urlId'] = $temp['id'];
        }
    ...
    public function strategy() {
        $strategyModel = model('Strategy');
        $user = $this->auth->getUser();
        $strategy = $strategyModel->getStrategyByUid($user['id']);
        $this->view->assign('strategy', $strategy);
        $this->view->assign('title', __('我的攻略'));
        return $this->view->fetch();
    }
    ...
    public function photo() {
        ...
        $user = $this->auth->getUser();
        $photo = $photoModel->getPhotoByUid($user['id']);
        $this->view->assign('photo', $photo);
        $this->view->assign('title', __('我的照片'));
        return $this->view->fetch();
    }
}
